...
class ChatRoomApiManager(object):
    ...
    def get_commands(self, token):
        messages_url = "{api_base_url}/api/chat/messages/".format(api_base_url=self.api_base_url)
        querystring = {"chat_room": "default"}
        headers = {'Authorization': 'Token {token}'.format(token=token)}
        response = requests.request("GET", messages_url, headers=headers, params=querystring)
        return [message for message in json.loads(response.text) if not message['processed_by_bot']
